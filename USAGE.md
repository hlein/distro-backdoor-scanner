...
Run `package_unpack_all.sh` followed by `package_scan_all.sh`.
...
Run `package_decompcheck_all.sh` after having run `package_unpack_all.sh`.
...
First run `populate_m4_db.sh` to create a reference database, then
run `MODE=1 find_m4.sh` after having run `package_unpack_all.sh` to
compare unpacked pacakges' `.m4` files to the known references.
...
using the same per-distro handling as `package_unpack_all.sh`). At the
end of the run, outputs lists of not-seen-before `.m4` files, and `git
